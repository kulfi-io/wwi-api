...
describe("Role Claim Query", () => {
    describe("Claim", () => {
        it("returns view role claim", async () => {
            ...
            expect(`${data.role} ${data.claim}`).equal("Basic User - updated View Claim");
        });
    });
    ...
    describe("Claims", () => {
        it("returns all claims", async () => {
            ...
            expect(`${update.role} ${update.claim}`).equal("Basic User - updated Update Claim");
            expect(`${add.role} ${add.claim}`).equal("Basic User - updated Add Claim");
        });
    });
});
